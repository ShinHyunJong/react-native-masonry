...
export default function Brick(props) {
  ...
  const image = props.onPress
    ? _getTouchableUnit(props, props.gutter)
    : _getImageTag(props, props.gutter);
...
export function _getTouchableUnit(image, gutter = 0) {
  return (
    <TouchableOpacity key={image.uri} onPress={() => image.onPress(image.data)}>
      <View>{_getImageTag(image, gutter)}</View>
    </TouchableOpacity>
  );
}
